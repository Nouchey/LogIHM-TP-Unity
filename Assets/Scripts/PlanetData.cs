...
public static class PlanetData
{
    ...
    public static Vector3 GetPlanetPosition(Planet p, DateTime t)
    {
        ...
        do
        {
            ...
            // Don't know why the series does not converge with order 2 term (see above)
    ...
    public static float GetPlanetRotation(Planet planet, DateTime t)
    {
        long minutes = (long)(t - new DateTime(1, 1, 1)).TotalMinutes;
        int revolutionPeriod;
        switch (planet)
        {
            case Planet.Mercury:
                revolutionPeriod = 1408;
                break;
            case Planet.Venus:
                revolutionPeriod = 5832;
                break;
            case Planet.Mars:
                revolutionPeriod = 25;
                break;
        ...
        double proportion = (double)(minutes % (revolutionPeriod * 60)) / (revolutionPeriod * 60);
        float angle = (float)(proportion * -360);
        if (planet == Planet.Earth)
        {
            angle += 53.75f; // correct orientation for that planet
            if (angle >= 360)
                angle -= 360; // yes I know modulos exist but I got type PTSD now!
        }
        return angle;
    }
